...
On desktop, both bands and the main-navigation surface are transparent so the parent header colour and artwork show through. A single divider remains below the utility band, between the secondary and main rows; the navigation band has no lower border. Their inner `.container` elements retain the page alignment. [`src/styles/compact-header.scss`](src/styles/compact-header.scss) is imported last and provides the desktop-only layout, logo/menu contrast, widened menu space, and the rule that hides the secondary-nav mobile accordion copy at `769px` and above. It also fixes the two-band split's header z-index stacking, pins the mega-menu flyout flush below the navigation band, and reworks its open/close animation to grow downward instead of sliding through the bars — see [src/styles/SQUIZ_MATRIX_ASSET_MAPPING.md](src/styles/SQUIZ_MATRIX_ASSET_MAPPING.md) for the full breakdown.

On desktop, the secondary-navigation links, main-navigation links, header logo, and current-user profile name use the secondary colour (`#2E979B`) on hover. The logo changes its SVG `fill`; the remaining links change their text `color`.

Secondary-navigation folder triggers use `href="#"` and toggle their submenu instead of navigating to a content page. External links keep their `.external` class and URL, but their generated Font Awesome external-link icon is suppressed within the secondary menu only.
